...
Resource percentage follow-up (Lotar, 2026-09-10):
- Scope: extend the existing resource controls with 0–100% inputs and retain exact GiB/core entry. Delete a new persisted percentage schema and global preset; existing absolute offers remain the enforcement contract. Keep existing OS reserves, model admission and RSS enforcement. CPU percentages round down to whole cores. Disk percentages use current free disk, retaining the existing warning-only absolute disk contract.
- The local API now applies the same missing-reserve fallback as validateOffer. All three OS tests exercise 0/25/50/100% and reject one MiB into the reserve without changing the saved offer.
- Browser exercised the real local API/UI in an isolated process using live Mac capabilities: untouched save preserves the exact offer; 0/25/50/100% save/reload passes; 100% RAM = 118784 MiB with 12288 MiB reserved; unsafe full-physical-RAM request returns 400. Evidence: /tmp/swarmlet-percentage-browser-result.json and /tmp/swarmlet-resource-percentages.png.
- Review verdict: merge. Correctness: corrected percent input step so displayed fractional percentages do not invalidate form saves; retained MiB precision. Contracts: app.js consumes existing localapi.ts absolute limits and validateOffer stays authoritative. Data safety: no migration; offer writes retain existing setOffer path. Time: no new time logic. Concurrency: live measurement polling does not overwrite unsaved controls. Security: no new routes or authority. Tests: actual browser saves plus cross-OS endpoint rejection. Simplicity: extended rangeRow; no duplicate percent implementation or storage.
- Fresh validation: `bun run --cwd swarmlet typecheck && bun test swarmlet/protocol swarmlet/control swarmlet/node-agent` -> 308 pass, 0 fail, 3779 expectations. Final model/allocation acceptance and installed artifact rollout remain pending.
- Follow-on requested sequence: install newest packages, run full model/allocation E2E and report, then deploy control at app.swarmlet.ai, migrate all nodes and exercise a real signed automatic update. User added DNS. Verified live server root@46.225.53.158 (hostname the-shop), existing Traefik and Swarmlet site containers healthy. Stale SSH alias shop points to a different host; use the verified address.
