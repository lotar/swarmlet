...
| Chat + live tps | Chat tab streams through the router and shows llama-server's own timings per reply; Nodes tab shows a live tok/s per node (router-counted streamed tokens, 5 s window) plus the agent's heartbeat rate; headless run: reply from production via the router with timings, Nodes column read "7.0 streaming now · qwen3.8-flash-next" mid-reply. Production itself measured 3.6–4.3 tok/s server-side during the test (host swapping 19 GB, another client in flight) | `app-chat-20260904T141000Z` |
| GUI e2e | macOS app launched (sidecar agent + bundled engine, connected); Linux shell on Legion 2\'s display found the service and opened the window; headless Chrome drove the node UI (offer edit + save, models, network measurement, logs) on the M5 and on Legion 1, and the control UI (nodes table, join code, deployment form → plan preview → create/start → ready → routing table → API key → request routed to the 3-node split → stop with confirm → events); zero console errors | `app-gui-e2e-20260904T124500Z` (screenshots + transcripts), `swarmlet/e2e/gui/` |
