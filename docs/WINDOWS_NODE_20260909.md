...
3. `bootstrap-toolchain.ps1` over SSH: Git, Bun, CMake, Ninja, rustup (MSVC toolchain), Visual Studio 2022 Build Tools with the C++ workload. `nvidia-smi -L` decides CPU or `-Cuda` for the engine build (CUDA toolkit needed for `-Cuda`).
...
The Mac-side work above is complete and committed. The laptop steps are NOT RUN: as of 2026-09-10 00:20 (Zagreb) the laptop still answers ARP but drops every inbound packet, which is the Windows Public-profile firewall state; the setup script has not been executed on it yet (the Mac's file server log shows no download from 192.168.1.188, and a `/24` sweep found no other host with port 22 or 3389 open). A watcher on the Mac records the moment port 22 opens (`/tmp/winbox-watch.log`). Rendered appearance of the Windows shell, the Mica effect, drag and caption behavior, the scheduled task, the power settings taking effect, the model hash on the laptop and the routed reply remain unverified until then.
