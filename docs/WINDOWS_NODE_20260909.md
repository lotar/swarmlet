...
Flash-Next routing (2026-09-12, 13:00): the split is stopped (it crashed 11 times overnight, once per WebSocket drop on any node, and bounced production each time). A whole-model replica on the Mac (`flashnext-m5`, `dep-19fb57bca9a0`) was refused by the Mac agent's memory fit gate (66 to 69 GiB free after stopping production, 79232 MiB needed; Docker Desktop VM and applications hold the rest). Selecting `qwen3.8-flash-next` on the Windows node lands on the standalone Mac server on 8099 again (`flashnext-prod`). Details, numbers and the owner's options: `docs/reports/FLASHNEXT_M5_REPLICA_20260912.md`.

Flash-Next routing (2026-09-12, 14:10): after the owner had all Docker containers stopped and Docker Desktop quit, the whole-model replica loaded. Selecting `qwen3.8-flash-next` on the Windows node now lands on `flashnext-m5-64k` (`dep-5a6b848b63d0`: the full model on the Mac GPU, ctx 65536, 4 slots); probe from this laptop 200 in 1.84 s, decode 35.4 tok/s, against 3.2 s through the production tunnel and 8.05 s through the split. 8099 is down while the replica serves and returns when the deployment is stopped. Same report for the remeasure table and the reversal steps.

Flash-Next routing (2026-09-12, 14:31): the replica was lost at 14:22 (controller re-placed it after a 7 s WebSocket blip on the Mac agent, then the memory fit gate refused twice) and is `stopped`. Selecting `qwen3.8-flash-next` on the Windows node lands on `flashnext-prod` (production 8099 through the Mac tunnel) again, as on 2026-09-11 before the split.
