...
The local gateway selects a ready coordinator or replica on its own machine first. A worker holds only some model layers and cannot decode by itself; its gateway sends inference through the same internet host as its authenticated agent connection. A local coordinator serving a split model still contacts its remote workers. No extra model copy is loaded. The response header `x-swarmlet-route` reports `local` or `mesh`. Model entries include the OpenAI-required `created` timestamp in Unix seconds (the served model registration time). To check SDK compatibility, run `uv run --with openai==3.8.0 python swarmlet/e2e/tools/openai-client.py --expect-route local` on a coordinator; workers use `--expect-route mesh`.
