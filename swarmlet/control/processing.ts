...
export function processingSnapshot(deps: ControlDeps, url: URL, now = Date.now()) {
  ...
  const phased = native?.mode === "prefill-decode";
  const total = profile?.layers ?? (native ? 24 : plan?.tensorSplit.reduce((a, b) => a + b, 0)) ?? null;
  const rows = native ? native.endpoints.map((e, i) => ({
    id: e.nodeId, role: phased ? (i === 0 ? "Prefill" : "Decode") : `Stage ${i + 1}`, device: e.device,
    layers: phased ? total : Number(e.identity.stage_end) - Number(e.identity.stage_start),
  })) : plan ? [
  ...
  return {
    ...
    shareBasis: plan && !phased ? "assigned_layers" : "whole_model", totalLayers: total,
    nodes: rows.map((row) => {
      ...
      return { ...row, name: n?.hostname ?? row.id.slice(0, 8), online,
        sharePct: phased ? null : !plan ? 100 : total && row.layers != null ? pct(row.layers / total * 100) : null,
