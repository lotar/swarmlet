...
struct options {
    ...
    uint32_t chain = 1;   // drafts per stream per hop (worker --chain must be >= this)
...
void usage(const char * argv0) {
    ...
    std::printf("  --chain K               chained drafts per stream per hop, 1..%u (default 1); remote arms only\n", MAX_CHAIN);
...
options parse_options(int argc, char ** argv, bool & help) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--chain") result.chain = parse_u32(value(i, arg.c_str()), "chain");
...
public:
    worker_client(const std::string & endpoint, const std::string & worker_id, const std::string & worker_digest,
        ...
        if (response.hidden_width != HIDDEN_WIDTH || response.vocab != vocab || (response.supported_features & REQUIRED_FEATURES) != REQUIRED_FEATURES ||
    ...
    rpc_result infer(std::vector<sequence_rows> sequences) {
        ...
        if (response.campaign_epoch != epoch_ || response.msg_id != request.msg_id || response.proposals.size() % request.sequences.size() != 0) {
        ...
        for (size_t i = 0; i < response.proposals.size(); ++i) {
            ...
            if (p.seq_id != request.sequences[i % request.sequences.size()].seq_id || p.seq_id >= MAX_SEQS || p.token < 0 || (uint32_t) p.token >= vocab_ ||
    ...
    void reset_seq(uint8_t seq) {
        std::lock_guard<std::mutex> lock(mutex_);
        const uint8_t mask = (uint8_t) (1u << seq);
        reset_request request{epoch_, next_msg_id_++, mask};
        send_frame(fd_.get(), message_type::reset_request, encode(request));
        const frame response_frame = recv_frame(fd_.get());
        if (response_frame.type != message_type::reset_response) throw std::runtime_error("worker seq reset failed");
        const reset_response response = decode_reset_response(response_frame.payload);
        if (response.msg_id != request.msg_id || response.sequence_mask != mask) throw std::runtime_error("worker seq reset mismatch");
    }
...
struct stream_state {
    ...
    llama_seq_id spec = 0;
    llama_seq_id spec_pad = 0;                 // scratch seq for equal-length padding entries during replay phases
    ...
    bool replay = false;                       // committed state lags: replay_tokens (ending before `current`) must be fed on committed
    std::vector<llama_token> replay_tokens;    // tokens at pos-len..pos-1 not yet inside committed state
    bool worker_resync = false;                // worker chained past a rejected draft; reset its sequence before the next draft
...
struct metrics {
    ...
    std::array<uint64_t, MAX_CHAIN + 1> accept_hist{}; // how many hops accepted exactly n drafts
};
...
struct target_context {
    ...
    target_context(const target_model & tm, const options & opts) {
        ...
        cp.kv_unified = true;
        cp.n_rs_seq = 0;
        cp.n_outputs_max = std::max((int) (1 + opts.chain)*N_STREAMS, N_SEQ_MAX); // output_reserve(n_seq_max) runs at construction
        cp.n_outputs_max_per_seq = 1 + opts.chain;
        cp.no_perf = false;
...
public:
    benchmark(const options & opts, const arm_spec & arm, const target_model & tm,
        ...
        for (int i = 0; i < N_STREAMS; ++i) {
            ...
            streams_[i].spec_pad = i + 2*N_STREAMS;
    ...
    std::string summary() const {
        ...
        char buf[512];
        std::snprintf(buf, sizeof(buf), "%s mode=%s groups=%u chain=%u tok/s=%.3f phases=%llu tokens=%llu phase p50/p95=%.1f/%.1f ms verify p50/p95=%.1f/%.1f ms wait p50/p95=%.1f/%.1f ms batch=%llu acc=%.4f (%llu/%llu, forced %llu) replay=%llu natural_mismatch_margin max=%.4f",
                arm_.label.c_str(), mode_name(), arm_.groups, opts_.chain, tokens_per_second(), (unsigned long long) timed_.phases, (unsigned long long) timed_.tokens,
...
private:
    ...
    void launch_drafts(int group) {
        ...
        for (int worker = 0; worker < N_WORKERS; ++worker) {
            ...
            for (auto & st : streams_) {
                ...
                std::vector<row> rows;
                if (st.worker_resync) {
                    // the worker advanced its own chain past a rejected draft: drop its sequence so positions restart at `pos`
                    clients_[worker]->reset_seq(st.worker_seq);
                    st.worker_resync = false;
                    st.catch_up.reset();
                }
                if (st.catch_up) {
    ...
    void phase(int group, const std::array<bool, N_STREAMS> * force_reject, bool timed) {
        ...
        std::array<std::vector<llama_token>, N_STREAMS> proposals{};
        ...
        if (use_remote()) {
            for (int worker = 0; worker < N_WORKERS; ++worker) {
                ...
                for (const auto & p : result.response.proposals) {
                    ...
                    if (proposals[seq].size() < opts_.chain) proposals[seq].push_back(p.token);
                }
            }
        } else if (use_oracle()) {
            for (int s = 0; s < N_STREAMS; ++s) if (drafting[s]) proposals[s].assign(1, oracle_proposal(streams_[s], forced[s]));
        }
        const uint64_t wait_us = now_us() - wait_start;
        for (int s = 0; s < N_STREAMS; ++s) {
            if (!drafting[s]) continue;
            if (proposals[s].empty()) throw std::runtime_error("no proposal for a drafting sequence");
            if (force_reject && (*force_reject)[s]) { proposals[s][0] = (proposals[s][0] + 1) % tm_.vocab; forced[s] = true; }
            while (proposals[s].size() < opts_.chain) proposals[s].push_back(proposals[s].back()); // pad: keeps every stream at 1+K batch tokens
        ...
        llama_batch batch = llama_batch_init((1 + (int) opts_.chain)*N_STREAMS, 0, 1);
        std::array<int32_t, N_STREAMS> idx_a{};
        std::array<std::vector<int32_t>, N_STREAMS> idx_b{};
        idx_a.fill(-1);
        for (int s = 0; s < N_STREAMS; ++s) {
            ...
            if (target_only()) {
                ...
                // replay entry: the known prefix (replay tokens) then current, all on committed; padded to 1+K entries
                expect_pos_max(st.committed, (llama_pos) st.pos - 1 - (llama_pos) st.replay_tokens.size(), "replay");
                for (size_t r = 0; r < st.replay_tokens.size(); ++r)
                    common_batch_add(batch, st.replay_tokens[r], (llama_pos) (st.pos - st.replay_tokens.size() + r), {st.committed}, false);
                common_batch_add(batch, st.current, (llama_pos) st.pos, {st.committed}, true);
                idx_a[s] = batch.n_tokens - 1;
                // unequal per-stream counts are fine: split_equal defers the odd sequence sets to further ubatches
            } else {
                common_batch_add(batch, st.current, (llama_pos) st.pos, {st.spec}, true);
                idx_a[s] = batch.n_tokens - 1;
                for (uint32_t k = 0; k < opts_.chain; ++k) {
                    common_batch_add(batch, proposals[s][k], (llama_pos) st.pos + 1 + k, {st.spec}, true);
                    idx_b[s].push_back(batch.n_tokens - 1);
                }
            }
        }
        ...
        for (int s = 0; s < N_STREAMS; ++s) {
            ...
            if (st.replay) {
                // replay entries decoded on the committed sequence: state is exact through `current`, no draft to judge
                row cu;
                cu.pos = st.pos;
                cu.token = st.current;
                cu.hidden = st.hidden;
            ...
            // longest accepted prefix: draft k accepted iff it equals the greedy of the previous position
            std::vector<llama_token> greedy_b(opts_.chain);
            std::vector<const float *> hidden_b(opts_.chain);
            for (uint32_t k = 0; k < opts_.chain; ++k) {
                greedy_b[k] = greedy(llama_get_logits_ith(ctx(), idx_b[s][k]), tm_.vocab);
                hidden_b[k] = llama_get_embeddings_nextn_ith(ctx(), idx_b[s][k]);
                if (!hidden_b[k]) throw std::runtime_error("target hidden output is unavailable");
            }
            uint32_t n_acc = 0;
            {
                llama_token expect = ya;
                for (uint32_t k = 0; k < opts_.chain; ++k) {
                    if (proposals[s][k] != expect) break;
                    ++n_acc; expect = greedy_b[k];
                }
            }
            for (uint32_t k = 0; k < opts_.chain; ++k) st.proposal_hash = fnv1a64(&proposals[s][k], sizeof(llama_token), st.proposal_hash);
            append_output(st, st.current);
            const uint64_t commit_start = now_us();
            if (n_acc == opts_.chain) {
                // full accept: speculative state is exact through pos+K
                for (uint32_t k = 0; k < opts_.chain && st.output.size() < opts_.tokens; ++k) append_output(st, proposals[s][k]);
                ...
                expect_pos_max(st.committed, (llama_pos) st.pos + (llama_pos) opts_.chain, "commit");
                // worker catch-up: it must see (pos+1 .. pos+K) rows; it already computed them itself in the chain, so only the last hidden matters
                row cu;
                cu.pos = st.pos + opts_.chain;
                cu.token = proposals[s][opts_.chain - 1];
                { const float * hprev = opts_.chain >= 2 ? hidden_b[opts_.chain - 2] : ha; cu.hidden.assign(hprev, hprev + HIDDEN_WIDTH); }
                st.catch_up = std::move(cu);
                st.hidden.assign(hidden_b[opts_.chain - 1], hidden_b[opts_.chain - 1] + HIDDEN_WIDTH);
                st.current = greedy_b[opts_.chain - 1];
                st.pos += 1 + opts_.chain;
            } else {
                // partial or zero accept: discard speculative; the accepted prefix (n_acc drafts) plus current are replayed on committed
                ...
                st.replay_tokens.clear();
                st.replay_tokens.push_back(st.current);
                for (uint32_t k = 0; k < n_acc; ++k) { st.replay_tokens.push_back(proposals[s][k]); if (st.output.size() < opts_.tokens) append_output(st, proposals[s][k]); }
                st.replay = true;
                const float * hlast = n_acc == 0 ? ha : hidden_b[n_acc - 1];
                st.hidden.assign(hlast, hlast + HIDDEN_WIDTH);
                st.current = n_acc == 0 ? ya : greedy_b[n_acc - 1];
                st.pos += 1 + n_acc;
                // worker must re-sync: it advanced K positions on its own chain; reset its sequence next launch
                st.worker_resync = true;
            }
            commit_us += now_us() - commit_start;
            if (timed) {
                timed_.proposals += opts_.chain;
                timed_.accepted += n_acc;
                if (n_acc < opts_.chain) {
                    if (forced[s] && n_acc == 0) ++timed_.forced_rejects;
                    else if (n_acc == 0) timed_.natural_mismatch_margin.push_back(la[ya] - la[proposals[s][0]]);
                    timed_.accept_hist[n_acc] += 1;
                } else timed_.accept_hist[n_acc] += 1;
            }
            phase_tokens += st.output.size() - before;
        }
        llama_batch_free(batch);
    ...
    void emit_json(FILE * f) const {
        ...
        std::fprintf(f, "  \"schedule\": {\"groups\": %u, \"streamsPerGroup\": %d, \"draftLaunch\": \"end-of-phase\", \"chain\": %u, \"acceptHist\": [%llu,%llu,%llu,%llu,%llu]},\n", arm_.groups, N_STREAMS / (int) arm_.groups, opts_.chain, (unsigned long long) timed_.accept_hist[0], (unsigned long long) timed_.accept_hist[1], (unsigned long long) timed_.accept_hist[2], (unsigned long long) timed_.accept_hist[3], (unsigned long long) timed_.accept_hist[4]);
