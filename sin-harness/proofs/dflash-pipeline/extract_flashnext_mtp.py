...
REPO='Qwen/Qwen3.8-Flash-Next';BASE='https://huggingface.co'
KEEP={'model.language_model.embed_tokens.weight','lm_head.weight'}
ap=argparse.ArgumentParser();ap.add_argument('--meta',default='/Users/lotar/projects/local-llm/models/qwen3.8-flash-next-hf');ap.add_argument('--out',default='/Users/lotar/projects/local-llm/models/qwen3.8-flash-next-mtp-source');ap.add_argument('--workers',type=int,default=2);ap.add_argument('--chunk-mib',type=int,default=64);ap.add_argument('--plan',action='store_true');a=ap.parse_args();META=Path(a.meta);OUT=Path(a.out);PARTS=OUT/'parts';OUT.mkdir(parents=True,exist_ok=True);PARTS.mkdir(exist_ok=True);lock=threading.Lock()
if shutil.disk_usage(OUT).free<40*2**30:raise SystemExit('need >=40GiB free')
api=json.load(urllib.request.urlopen(f'{BASE}/api/models/{REPO}'));rev=api['sha'];local_idx=json.load(open(META/'model.safetensors.index.json'));remote_raw=urllib.request.urlopen(f'{BASE}/{REPO}/resolve/{rev}/model.safetensors.index.json').read();remote_idx=json.loads(remote_raw)
...
def header_for(shard):
 url=f'{BASE}/{REPO}/resolve/{rev}/{shard}';tmp=OUT/f'.hdr-{shard}'
...
def download(name):
 ...
 url=f'{BASE}/{REPO}/resolve/{rev}/{s["shard"]}';chunk=a.chunk_mib*2**20
 while have<s['bytes']:
...
ordered=sorted(specs);offset=0;head={'__metadata__':{'format':'pt','source':REPO,'revision':rev,'scope':'mtp+embed+head'}}
for name in ordered:
...
manifest={'repo':REPO,'revision':rev,'bytes':offset,'tensors':[{**specs[k],'sha256':hashes[k],'part':part_path(k).name} for k in ordered],'consolidated':final.name,'consolidatedSha256':None}
