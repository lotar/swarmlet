...
ap=argparse.ArgumentParser();ap.add_argument('--url',default='http://127.0.0.1:8095');ap.add_argument('--out',required=True);ap.add_argument('--baseline');ap.add_argument('--tokens',type=int,default=128);a=ap.parse_args();out=Path(a.out);out.mkdir(parents=True,exist_ok=True);rows=[]
for i,prompt in enumerate(PROMPTS):
 body={'prompt':prompt,'n_predict':a.tokens,'temperature':0,'top_k':1,'top_p':1.0,'seed':42,'cache_prompt':False,'ignore_eos':True}
 ...
 wall=time.perf_counter()-t;row={'index':i,'prompt':prompt,'promptSha':__import__('hashlib').sha256(prompt.encode()).hexdigest(),'tokens':data.get('tokens'),'content':data.get('content'),'timings':data.get('timings',{}),'wallSeconds':wall};rows.append(row);(out/f'{i}.json').write_text(json.dumps(row,indent=2,sort_keys=True))
if a.baseline:
 base=json.loads(Path(a.baseline).read_text())['rows']
 for b,r in zip(base,rows):
  assert b['tokens']==r['tokens'],f'token mismatch prompt {r["index"]}'
  assert b['content']==r['content'],f'content mismatch prompt {r["index"]}'
summary={'rows':rows,'medianDecodeTps':statistics.median(r['timings'].get('predicted_per_second',0) for r in rows),'medianWallSeconds':statistics.median(r['wallSeconds'] for r in rows)};(out/'summary.json').write_text(json.dumps(summary,indent=2,sort_keys=True));print('RESULT_JSON='+json.dumps({'medianDecodeTps':summary['medianDecodeTps'],'medianWallSeconds':summary['medianWallSeconds'],'prompts':len(rows)},separators=(',',':')))
