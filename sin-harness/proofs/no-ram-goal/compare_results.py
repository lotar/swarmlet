...
lines+=['','## Best known points','',f"- Best measured full Qwen: **{max(x['tps'] for x in rows if x['category']=='Qwen full model'):.2f} tok/s** (native MTP n=3).",'- 50 tok/s remains a conditional rack-scale pipeline simulation, not an empirical result.','- Pan-European token-path variants remain below target.','- Kimi disk streaming fits memory but is not interactive.','']
text='\n'.join(lines)
if a.out:Path(a.out).write_text(text)
print(text);print('RESULT_JSON='+json.dumps({'rows':len(rows),'bestMeasuredQwenTps':max(x['tps'] for x in rows if x['category']=='Qwen full model'),'simulated50Found':any(x['kind']=='simulated' and x['tps']>=50 for x in rows)},separators=(',',':')))
