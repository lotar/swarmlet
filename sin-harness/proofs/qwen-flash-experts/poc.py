...
SHARD=Path(os.environ.get('QWEN_SHARD','/Users/lotar/projects/local-llm/models/qwen3.8-flash-next/UD-Q4_K_XL/Qwen3.8-Flash-Next-UD-Q4_K_XL-00002-of-00005.gguf'));HERE=Path(__file__).parent;SERVICE_PORT=9590
...
def main():
 ...
 refcell=QwenExpertCell(SHARD,HERE/'worker.py');X=deterministic_activation()[None,:];ids,w=softmax_top10(X,refcell.router);reference=streamed_reference(refcell.T,X,ids,w)
 ...
 server=subprocess.Popen([sys.executable,str(HERE/'server.py'),'--port',str(SERVICE_PORT),'--shard',str(SHARD)],env=env,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
 try:
  for _ in range(600):
  ...
  manifest=req('/manifest');epoch=manifest['placementEpoch'];assert manifest['selectedExperts']==refcell.selected and manifest['placement']==refcell.placement;assert len(epoch)==64 and manifest['epochSigned'] is False;assert all(len(x)==64 for x in manifest['nodeContentDigests'].values())
  print('[qwen-service] epoch',epoch[:16],'placement',manifest['placement'],flush=True)
  def forward(Xb,profile,ep=epoch):
  ...
  actual,_,_=forward(X,'lan');max_abs=float(np.max(np.abs(actual-reference)));max_rel=float(np.max(np.abs(actual-reference)/np.maximum(np.abs(reference),1e-6)));assert max_abs<2e-5 and max_rel<2e-4,(max_abs,max_rel)
  ...
  ref4=streamed_reference(refcell.T,X4,ids4,w4);act4,_,_=forward(X4,'lan');batch4_abs=float(np.max(np.abs(act4-ref4)));assert batch4_abs<2e-5,batch4_abs
  ...
  try:forward(X,'lan','0'*64);raise AssertionError('stale public epoch accepted')
  ...
  try:forward(outsider,'lan');raise AssertionError('nonresident route accepted')
  except urllib.error.HTTPError as e:assert e.code==409

  benches={}
  for profile in ['lan','eu']:
   for B in [1,4,16]:
    Xb=X*np.linspace(.85,1.15,B,dtype=np.float32)[:,None];samples=[];inside=[]
    for _ in range(3):_,ms,im=forward(Xb,profile);samples.append(ms);inside.append(im)
    med=float(np.median(samples));imed=float(np.median(inside));benches[f'{profile}_b{B}']={'apiMedianMs':med,'internalMedianMs':imed,'throughput':B*1000/med};print(f'[qwen-service bench] {profile} batch={B} api={med:.1f}ms internal={imed:.1f}ms throughput={B*1000/med:.2f}tok/s',flush=True)
  assert benches['eu_b1']['internalMedianMs']-benches['lan_b1']['internalMedianMs']>=15

  req('/admin/stop-owner',{'nodeId':'n2','placementEpoch':epoch});failed=False
  try:forward(X,'lan')
  except urllib.error.HTTPError as e:failed=e.code==503
  assert failed
  req('/admin/start-owner',{'nodeId':'n2','placementEpoch':epoch});retry,_,_=forward(X,'lan');assert float(np.max(np.abs(retry-reference)))<2e-5
  ...
  eu=benches['eu_b1']['apiMedianMs'];summary={**manifest,'fullFfnParityMaxAbs':max_abs,'fullFfnParityMaxRel':max_rel,'batch4ParityMaxAbs':batch4_abs,'benchmarks':benches,'projected48LayerEuMsPerToken':eu*48,'projected48LayerEuTokPerSec':1000/(eu*48),'proofFinalIncrementalRssMiB':proof_rss/1024,'peakAggregateRssDeltaMiB':peak[0]/1024,'swapDeltaMiB':swap,'staleEpochFailClosed':True,'nonresidentRouteFailClosed':True,'ownerLossFailClosed':True,'restartParity':True,'scope':'complete layer-0 FFN (routed + shared), excluding attention/SSM/residual/KV/logits'}
