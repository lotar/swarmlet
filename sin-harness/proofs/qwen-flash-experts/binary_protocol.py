...
def encode_service_request(X,profile,epoch):
 ...
 if X.ndim!=2 or X.shape[1]!=HIDDEN or not 1<=len(X)<=MAX_BATCH or not np.isfinite(X).all():raise ValueError('bad activation shape')
...
def decode_service_request(raw):
 ...
 if not np.isfinite(X).all():raise ValueError('non-finite activations')
 return X,PROFILE_IDS[p],e.hex()
def encode_service_response(Y,epoch):
 Y=np.asarray(Y,dtype=np.float16)
 if Y.ndim!=2 or Y.shape[1]!=HIDDEN or not 1<=len(Y)<=MAX_BATCH or not np.isfinite(Y).all():raise ValueError('bad service response')
 b,h=Y.shape
...
def decode_service_response(raw):
 ...
 if m!=b'QFFR' or v!=VERSION or h!=HIDDEN or not 1<=b<=MAX_BATCH:raise ValueError('bad response header')
 if len(raw)!=SRESP.size+b*h*2:raise ValueError('bad response length')
 Y=np.frombuffer(raw,dtype='<f2',offset=SRESP.size,count=b*h).astype(np.float32).reshape(b,h)
 if not np.isfinite(Y).all():raise ValueError('non-finite response')
 return Y,e.hex()
def encode_worker_request(X,assignments,epoch,delay_ms=0):
 X=np.asarray(X,dtype=np.float16)
 if X.ndim!=2 or X.shape[1]!=HIDDEN or not 1<=len(X)<=MAX_BATCH or not np.isfinite(X).all():raise ValueError('bad activation shape')
 b,h=X.shape;n=len(assignments)
 if not 1<=n<=10 or not 0<=delay_ms<=100:raise ValueError('bad worker request')
 ...
 for item in assignments:
  ...
  if w.shape!=(b,) or not np.isfinite(w).all():raise ValueError('bad gate weights')
...
def decode_worker_request(raw):
 ...
 for _ in range(n):
  ...
  eid=struct.unpack_from('<H',raw,off)[0];off+=2;w=np.frombuffer(raw,dtype='<f2',offset=off,count=b).astype(np.float32);off+=2*b
  if not np.isfinite(w).all():raise ValueError('non-finite gate weights')
  assign.append({'expertId':eid,'weights':w})
...
def encode_worker_response(partial,epoch):
 Y=np.asarray(partial,dtype=np.float16)
 if Y.ndim!=2 or Y.shape[1]!=HIDDEN or not 1<=len(Y)<=MAX_BATCH or not np.isfinite(Y).all():raise ValueError('bad worker response')
 b,h=Y.shape
 return WRESP.pack(b'QER1',VERSION,b,h,_epoch_bytes(epoch))+Y.astype('<f2',copy=False).tobytes()
def decode_worker_response(raw):
 if len(raw)<WRESP.size:raise ValueError('short worker response')
 m,v,b,h,e=WRESP.unpack_from(raw)
 if m!=b'QER1' or v!=VERSION or h!=HIDDEN or not 1<=b<=MAX_BATCH or len(raw)!=WRESP.size+b*h*2:raise ValueError('bad worker response')
 Y=np.frombuffer(raw,dtype='<f2',offset=WRESP.size,count=b*h).astype(np.float32).reshape(b,h)
 if not np.isfinite(Y).all():raise ValueError('non-finite worker response')
 return Y,e.hex()
