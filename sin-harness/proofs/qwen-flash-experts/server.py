...
import numpy as np
from cell import QwenExpertCell,NODE_IDS
ap=argparse.ArgumentParser();ap.add_argument('--port',type=int,default=9590);ap.add_argument('--shard',required=True);a=ap.parse_args();cell=QwenExpertCell(a.shard,Path(__file__).with_name('worker.py'))
class H(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if self.path=='/manifest':return self.sendj(200,cell.manifest())
 ...
 def do_POST(self):
  try:
   ...
   if self.path in ('/admin/stop-owner','/admin/start-owner'):
    ...
    if node not in NODE_IDS:return self.sendj(400,{'error':'BAD_NODE'})
...
try:
 cell.start();print('RESULT_SERVICE='+json.dumps({'port':a.port,**cell.manifest()},separators=(',',':')),flush=True);srv.serve_forever()
