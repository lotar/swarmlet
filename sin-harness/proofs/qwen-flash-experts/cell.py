...
NODE_IDS=['n1','n2','n3'];DEFAULT_PORTS=[9581,9582,9583]
...
class QwenExpertCell:
 def __init__(self,shard,worker_script,ports=DEFAULT_PORTS,python=sys.executable,gguf_py=None):
  self.shard=Path(shard);self.worker_script=Path(worker_script);self.ports=list(ports);self.python=python;self.gguf_py=Path(gguf_py or os.environ.get('GGUF_PY','/Users/lotar/projects/local-llm/llama.cpp-rpc/gguf-py'));self.lock=threading.RLock();self.procs={};self.closed=False
  ...
  ids,_=softmax_top10(deterministic_activation()[None,:],self.router);self.selected=[int(i) for i in ids[0]];self.placement={NODE_IDS[i]:self.selected[i::3] for i in range(3)};self.owner={eid:n for n,es in self.placement.items() for eid in es}
  self.expert_digests={str(e):expert_content_digest(self.T,e) for e in self.selected};self.node_digests={n:hashlib.sha256(json.dumps({str(e):self.expert_digests[str(e)] for e in sorted(es)},sort_keys=True,separators=(',',':')).encode()).hexdigest() for n,es in self.placement.items()}
  content={'model':'Qwen3.8-Flash-Next-UD-Q4_K_XL','shardBytes':self.shard.stat().st_size,'layer':0,'placement':self.placement,'expertDigests':self.expert_digests,'routerDigest':_hash_tensors([(ROUTER,self.T[ROUTER],self.T[ROUTER].data,None)]),'sharedDigest':_hash_tensors([(n,self.T[n],self.T[n].data,None) for n in SHARED]+[(SHARED_GATE,self.T[SHARED_GATE],self.T[SHARED_GATE].data,None)])};self.epoch=hashlib.sha256(json.dumps(content,sort_keys=True,separators=(',',':')).encode()).hexdigest()
 def manifest(self):return {'model':'Qwen3.8-Flash-Next-UD-Q4_K_XL','layer':0,'expertsTotal':512,'topK':10,'selectedExperts':self.selected,'placement':self.placement,'placementEpoch':self.epoch,'epochSigned':False,'nodeContentDigests':self.node_digests,'profiles':{'lan':LAN,'eu':EU}}
 def start_owner(self,node):
  with self.lock:
   ...
   i=NODE_IDS.index(node);env=os.environ.copy();env.update({'PYTHONPATH':str(self.gguf_py),'OMP_NUM_THREADS':'1','OPENBLAS_NUM_THREADS':'1','VECLIB_MAXIMUM_THREADS':'1'});cmd=[self.python,str(self.worker_script),'--id',node,'--port',str(self.ports[i]),'--shard',str(self.shard),'--experts',','.join(map(str,self.placement[node])),'--epoch',self.epoch]
   ...
   try:
    for _ in range(400):
     ...
     try:
      ...
      if m['nodeId']!=node or m['expertIds']!=sorted(self.placement[node]) or m['placementEpoch']!=self.epoch or m['ownedContentDigest']!=self.node_digests[node]:raise RuntimeError(f'{node} manifest mismatch')
      return
 ...
 def start(self):
  started=[]
  try:
   for n in NODE_IDS:self.start_owner(n);started.append(n)
  except Exception:
 ...
 def forward(self,X,profile,expected_epoch):
  ...
  def call(n):
   i=NODE_IDS.index(n);return post(f'http://127.0.0.1:{self.ports[i]}/execute',{'placementEpoch':self.epoch,'activations':X.tolist(),'assignments':groups[n],'delayMs':delays[n]})
  t=time.perf_counter()
  try:
